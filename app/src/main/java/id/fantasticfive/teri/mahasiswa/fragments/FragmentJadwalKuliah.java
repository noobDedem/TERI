...
import id.fantasticfive.teri.mahasiswa.adapters.JadwalKuliahAdapter;
import id.fantasticfive.teri.R;


public class FragmentJadwalKuliah extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
        ...
        adapter = new JadwalKuliahAdapter(getContext(), jadwalKuliahArray);
        ...
        if (item.getTitle().equals("Pasang Pengingat")) {
            if (jadwalKuliahArray.get(position).getNotifID() == 0) {
                ...
                if (id == 0){
                    editor.putInt("ID", 1);
                    editor.commit();
                    jadwalKuliahArray.get(position).setNotifID(1);
                } else {
                    id += 1;
                    editor.putInt("ID", id);
                    editor.commit();
                    jadwalKuliahArray.get(position).setNotifID(id);
                }
            }
             ...
             * Buat masang alarm
             */
            alarmReceiver.setAlarm(context, "Jadwal Kuliah",
                    jadwalKuliahArray.get(position).getHariMataKuliah(),
                    jadwalKuliahArray.get(position).getWaktuKuliah(),
                    jadwalKuliahArray.get(position).getNamaMataKuliah(),
                    jadwalKuliahArray.get(position).getRuangMataKuliah(),
            ...
            if (jadwalKuliahArray.get(position).getNotifID() == 0) {
                Toast.makeText(context, "Pengingat belum diset", Toast.LENGTH_LONG);
            } else {
                alarmReceiver.cancelAlarm(context, jadwalKuliahArray.get(position).getNotifID());
                jadwalKuliahArray.get(position).setNotifID(0);
                Toast.makeText(context, "Pengingat berhasil dihapus", Toast.LENGTH_LONG);
            }
    ...
    private boolean kelolaMatkul() {
        ...
        SharedPreferences.Editor editor = sharedPreferences.edit();
        boolean flagMatkul = sharedPreferences.getBoolean("SIMPAN", false);
